...
describe('AndroidBuild.ts', () => {
    ...
    it('emptyAlias.tc', () => {
        ...
        androidCommon.AndroidFieldsFill("TestApp", "1.1.1", "com.testandroid.io", "TestName", "", icon);
        
        // click on build button
        this.submitBtn = element(by.buttonText('Build')).click();
        
        let error = androidCommon.Alias.element(by.xpath("../div[@class='validation error ng-scope']"));
        ...
        browser.wait(EC.visibilityOf(error), 15000,'Wait for error element to appear').then (() =>
        {
           ...
           {
               expect(text).toBe("Alias is required");
           });
