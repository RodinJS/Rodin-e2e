...
describe('ViveBuild.ts', () => {
    ...
    it('spacesAsViveportID.tc', () => {
        ...
        viveCommon.viveFieldsFill("AppTest", "1.1.1", true, ' ', "key");

        let error = viveCommon.viveportID.element(by.xpath("../div[@class='validation error ng-scope']"));
        ...
        browser.wait(EC.visibilityOf(error), 15000,'Wait for error element to appear').then (() =>
        {
           ...
           {
               expect(text).toBe("Viveport id is required");
           });
